...
async function local(code) {
  const cmd = { networkId: C.networkId, payload: { exec: { code, data: {} } }, signers: [], meta: { chainId: C.chain, sender: '', gasLimit: 150000, gasPrice: 1e-8, ttl: 60, creationTime: Math.floor(Date.now() / 1000) }, nonce: String(Date.now()) };
...
async function swap({ dir, amountIn, account, publicHex, secretHex, slippage }) {
  ...
  const cmdObj = { networkId: C.networkId, payload: { exec: { code, data: envData } }, signers: [{ pubKey: publicHex, clist }], meta: { chainId: C.chain, sender: account, gasLimit: C.gasLimit, gasPrice: C.gasPrice, ttl: C.ttl, creationTime: Math.floor(Date.now() / 1000) }, nonce: 'monedero-swap:' + Date.now() };
