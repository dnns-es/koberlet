...
async function getTokenBalances(account, { node, networkId, tokens }) {
  ...
  return Promise.all(tokens.map(async (tk) => {
    ...
    if (safeMod) {
      ...
      // `chains` acota la busqueda a donde vive el token (los del DCA, solo en la 2).
      const donde = Array.isArray(tk.chains) && tk.chains.length ? tk.chains : CHAINS;
      await Promise.all(donde.map(async (ch) => {
        try {
...
// requiere que tenga algo de KDA en la chain destino).
async function transferCrossChain({ node, networkId, sourceChain, targetChain, from, to, amount, secretHex, publicHex, gasPayer, onProgress }) {
  assertKdaAccount(from, 'origen'); assertKdaAccount(to, 'destino');
  if (!to.startsWith('k:')) throw new Error('XC_ERR_DESTINO_K');
  if (String(sourceChain) === String(targetChain)) throw new Error('XC_ERR_MISMA_CHAIN');
  const amt = Number(amount); if (!(amt > 0)) throw new Error('XC_ERR_CANTIDAD');
  const canon = canonDecimal(amt);
  // Los avisos del cross-chain salen de aquí como CODIGO|dato|dato y el renderer los arma en
  // el idioma de la ventana. Antes eran frases en español a pelo, así que quien tenía Koberlet
  // en inglés recibía español justo en los mensajes que más importan (dinero en movimiento).
  ...
  // El pactId se enseña AQUÍ, antes de esperar nada: es lo único con lo que se remata un
  // envío que se quede a medias, y hasta la 2.11.1 se perdía si la espera vencía.
  say('XC_P_SALIDA_OK|' + pactId);

  // Esperar el minado del paso 1. Antes eran 20×3 s = 1 minuto, y en mainnet un bloque son
  // ~30 s: con la chain algo lenta la espera vencía con el envío perfectamente en marcha y
  // la app cantaba un fallo que no existía. 60×5 s = 5 minutos.
  const r1 = await pollResult({ node, networkId, chain: sourceChain, requestKey: pactId, tries: 60, delayMs: 5000 });
  // Hay que separar dos cosas que antes se contaban igual y no lo son:
  //   - sin respuesta -> NO ha fallado: la salida está en la cadena y se minará; solo que
  //     nosotros dejamos de mirar. Reenviar aquí es mandar el dinero dos veces.
  //   - failure       -> eso sí es un fallo de verdad, y lo dice la cadena.
  if (!r1 || !r1.result) {
    throw new Error('XC_ESPERA_SALIDA|' + pactId);
  }
  if (r1.result.status !== 'success') {
    throw new Error('XC_FALLO_SALIDA|' + ((r1.result.error && r1.result.error.message) || '?') + '|' + pactId);
  }
  ...
  // Sin prueba SPV tampoco se ha perdido nada: el dinero ya salió y el pacto sigue abierto.
  if (!proof) throw new Error('XC_ESPERA_SPV|' + targetChain + '|' + pactId);

  say('XC_P_ACUNANDO|' + targetChain);
  ...
  const r2 = await pollResult({ node, networkId, chain: targetChain, requestKey: j.requestKeys[0], tries: 60, delayMs: 5000 });
  // Mismo criterio que en la salida: quedarse sin respuesta no es haber fallado.
  if (!r2 || !r2.result) {
    throw new Error('XC_ESPERA_ACUNAR|' + targetChain + '|' + pactId);
  }
  if (r2.result.status !== 'success') {
    throw new Error('XC_FALLO_ACUNAR|' + ((r2.result.error && r2.result.error.message) || '?') + '|' + pactId);
  }
...
// Deja un colchón RESERVE de KDA por chain para el gas.
async function sendSmart({ node, networkId, targetChain, from, to, amount, secretHex, publicHex, gasPayer, onProgress }) {
  ...
  if (stillNeed > 0) {
    ...
    if (disponible < need - 1e-6) throw new Error('XC_ERR_SIN_SALDO|' + disponible.toFixed(4));
    ...
    if (stillNeed > 1e-6) throw new Error('XC_ERR_NO_JUNTA|' + stillNeed.toFixed(4));
  }

  say('XC_P_ENVIANDO|' + need + '|' + tgt);
  ...
  if (res && res.result && res.result.status === 'failure') {
    throw new Error('XC_ERR_ENVIO_FINAL|' + ((res.result.error && res.result.error.message) || '?'));
  }
...
// Firma un comando Pact que ha montado OTRO (una dApp por WalletConnect). Aqui no se
// interpreta lo que el comando hace: eso lo enseña la ventana y lo aprueba una persona.
// Lo unico que se garantiza es que se firma EXACTAMENTE el texto recibido, sin volver a
// serializarlo -un JSON.parse + JSON.stringify por el medio cambiaria el hash y la firma
// valdria para otro comando distinto del que se enseño-.
function firmarCmd(cmdStr, secretHex) {
  const { bytes, b64url } = hashCmd(String(cmdStr));
  return { sig: Buffer.from(nacl.sign.detached(bytes, secretKey64(secretHex))).toString('hex'), hash: b64url };
}

// `hashCmd` sale fuera porque WalletConnect lo necesita: al firmar un comando que llega
// de una dApp hay que devolverle su hash, y tiene que ser EL MISMO blake2b que usa el
// resto del monedero. Reimplementarlo aparte era la forma segura de que un día dejaran
// de coincidir sin que nadie se enterara.
module.exports = { enviarNft, simularFirmada, getBalance, getTokenBalances, transfer, transferToken, transferCreate, transferCrossChain, sendSmart, sendSigned, pollResult, local, validKdaAccount, assertKdaAccount, canonDecimal, hashCmd, firmarCmd };
