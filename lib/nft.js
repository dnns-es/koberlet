...
const kda = require('./kda');   // validador de cuentas Kadena (unico y contrastado)
...
// Cuenta: se delega en lib/kda.js, que es el validador contrastado contra la cadena
// (los 7 tipos de principal + cuentas antiguas con nombre, charset LATIN1 3..256).
// Antes aqui solo pasaban k:/w:, un criterio distinto al del envio de KDA.
const MODULO_OK = /^[A-Za-z0-9_\-.]{3,120}$/;

function idValido(id) { return typeof id === 'string' && ID_OK.test(id); }
function cuentaValida(c) { return kda.validKdaAccount(c); }
