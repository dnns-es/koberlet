...
// Saldo de una cuenta en los tokens del mercado, para la tarjeta de la wallet. De 12 en
// 12 y cada uno en su `try`: asi son seis lecturas y no setenta. Pero un contrato roto
// como lago.USD2 (ver CODE_MERCADO) tumba su tanda entera sin que `try` lo pare; esa
// tanda se repite token a token y el que falle en la CADENA se aparta para toda la
// sesion. Un fallo de red no aparta nada: la proxima vez se vuelve a preguntar.
const MOD_OK = /^[A-Za-z0-9_.-]+$/;
const CUENTA_OK = /^[A-Za-z0-9:_.-]{3,256}$/;
const ROTOS = new Set();
async function saldosMercado(cfg, cuenta, modulos) {
  if (!CUENTA_OK.test(String(cuenta))) return {};
  const lee = async (lote) => {
    const code = '[' + lote.map(m => '(try -1.0 (' + m + '.get-balance "' + cuenta + '"))').join(' ') + ']';
    const r = await local(cfg, code);
    if (r.status !== 'success' || !Array.isArray(r.data) || r.data.length !== lote.length) {
      const e = new Error('lote'); e.rota = r.status === 'failure'; throw e;
    }
    return r.data.map(num);
  };
  const mods = [...new Set(modulos)].filter(m => MOD_OK.test(m) && m !== KDA && !ROTOS.has(m));
  const lotes = [];
  for (let i = 0; i < mods.length; i += 12) lotes.push(mods.slice(i, i + 12));
  const out = {};
  await Promise.all(lotes.map(async (lote) => {
    try { (await lee(lote)).forEach((v, j) => { if (v > 0) out[lote[j]] = v; }); }
    catch (_) {
      for (const m of lote) {
        try { const [v] = await lee([m]); if (v > 0) out[m] = v; }
        catch (e) { if (e.rota) ROTOS.add(m); }
      }
    }
  }));
  return out;
...
module.exports = {
  mercado, cotizar, cambiar, cantidadPara, precisionDe, reparto, saldosMercado,
