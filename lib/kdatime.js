...
// Chainweb valida el creationTime de la tx contra el block-time del nodo. Pedimos ese block-time,
// calculamos el desfase con el reloj local y lo aplicamos: así la tx nunca "va al futuro" aunque
// el PC tenga la hora mal. Cachea el desfase por nodo (2 min).
const blake = require('blakejs');

const _off = {}; // { [node]: { at, offset } }  offset = segundos(nodo) - segundos(local)
...
// Desfase cacheado con el nodo. Si no se puede leer, cae a 0 (usará solo el buffer local).
async function offsetFor(node, networkId, chain) {
  const c = _off[node];
  if (c && Date.now() - c.at < 120000) return c.offset;
  try { const offset = await fetchOffset(node, networkId, chain); _off[node] = { at: Date.now(), offset }; return offset; }
