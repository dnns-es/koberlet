...
function friendly(e) {
  ...
  if (/blind signing|contract data/i.test(s)) return new Error('Activa "Blind signing" en el Ledger: app Ethereum → Settings → Blind signing → Enabled. Luego reintenta.');
...
async function sendToken({ index, rpc, token, to, amount }) {
  ...
  try {
    ...
    return await _sendEvmTx({ index, rpc, to: token, valueWei: 0n, data });
  } finally { pr.destroy(); }
}
