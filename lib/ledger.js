...
async function _sendEvmTx({ index, rpc, to, valueWei, data }) {
  ...
  const from = await withDevice(async (t) => (await new EthApp(t).getAddress(ethPath(index), false, false)).address);
  ...
  const sig = await withDevice(async (t) => new EthApp(t).signTransaction(ethPath(index), unsigned, resolution));
...
module.exports = { getAccount, signKdaTransfer, sendNative, sendToken };
