...
async function firmarYEnviar(cfg, opciones) {
  ...
  // Firma SIN ACOTAR (sin clist): la gasolinera no cabe, porque su GAS_PAYER obliga a
  // acotar la firma, y acotada no vale para un enforce-guard del dueño (ver `accion`).
  const g = opciones.sinAcotar ? null : await gasolinera(cfg);
  ...
  const cmd = {
    ...
    signers: [opciones.sinAcotar ? { pubKey: opciones.publicHex } : { pubKey: opciones.publicHex, clist: clist }],
    meta: {
...
// Pausar, reanudar y cerrar exigen la firma del dueño (enforce-guard del plan, FUERA de
// cualquier capability). Una firma acotada a coin.GAS no la cumple: el contrato devolvia
// "Keyset failure" y el plan no se podia cerrar (25/09/2026, comprobado en cadena). Por
// eso va SIN ACOTAR, y el gas lo paga el dueño (unos 0,000004 KDA). Es seguro porque el
// codigo que se firma es una sola llamada fija al contrato, armada aqui.
// Al cerrar, el contrato devuelve el bote restante.
async function accion(cfg, o) {
  ...
  return firmarYEnviar(cfg, {
    ...
    sinAcotar: true,
    from: o.owner, secretHex: o.secretHex, publicHex: o.publicHex, gasLimit: 2000
  });
}
