  ...
  for (let i = 0; i < 18; i++) {
    ...
    try {
      ...
      if (res) {
        ...
        // El dispatch devuelve el ID del mensaje (32 bytes en base64url). Con el se le puede
        // preguntar al mailbox de Ethereum si el rele ya lo entrego, en vez de adivinarlo por saldos.
        return { requestKey: rk, minedOk: true, messageId: messageIdHex(res.data) };
      }
    } catch (_) {}
  }
...
async function sendEvm2Kda({ rpc, secretHex, ledgerIndex, router, token, kadenaAccount, amount, decimals, kadenaDomain, kadenaChain }, onStep = () => {}) {
  ...
  try {
    ...
    out.messageId = messageIdDeRecibo(rcpt);
    return out;
  } finally { pr.destroy(); }
}
...
// Un mensaje del puente tiene un ID (keccak256 del mensaje, 32 bytes) que es EL MISMO en las dos
// orillas; solo cambia como se escribe: Ethereum lo maneja en hex (bytes32) y el mailbox Pact
// lo guarda en base64url (verificado el 2026-09-13 contra la tabla `deliveries` del fork).
// Aqui dentro se maneja siempre en hex con 0x; se convierte al preguntar a Kadena.
function messageIdHex(v) {
  if (v === null || v === undefined) return null;
  const s = String(typeof v === 'object' ? (v.data ?? '') : v).trim();
  if (/^(0x)?[0-9a-fA-F]{64}$/.test(s)) return '0x' + s.replace(/^0x/, '').toLowerCase();
  if (/^[A-Za-z0-9_-]{43}$/.test(s)) { const b = Buffer.from(s, 'base64url'); if (b.length === 32) return '0x' + b.toString('hex'); }
  return null;
}
function messageIdB64(hex) {
  const h = messageIdHex(hex); if (!h) return null;
  return Buffer.from(h.slice(2), 'hex').toString('base64url');
}
// En Ethereum el mailbox emite DispatchId(bytes32 indexed messageId) al despachar. Se saca del
// recibo de la transaccion transferRemote (verificado con una tx real del puente el 2026-09-13).
const TOPIC_DISPATCH_ID = ethers.id('DispatchId(bytes32)');
function messageIdDeRecibo(rcpt) {
  const l = ((rcpt && rcpt.logs) || []).find(x => x.topics && x.topics[0] === TOPIC_DISPATCH_ID && x.topics[1]);
  return l ? messageIdHex(l.topics[1]) : null;
}
// Recuperar el messageId de un envio ya hecho (para entradas viejas del historial sin el):
// Kadena→EVM desde su requestKey (/poll, result.data), EVM→Kadena desde su hash (recibo).
async function messageIdDeRequestKey({ node, networkId, chain, requestKey }) {
  try {
    const base = `${node}/chainweb/0.0/${networkId}/chain/${chain}/pact/api/v1`;
    const p = await fetch(base + '/poll', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestKeys: [requestKey] }) });
    const pj = await p.json(); const res = pj && pj[requestKey] && pj[requestKey].result;
    return (res && res.status === 'success') ? messageIdHex(res.data) : null;
  } catch (_) { return null; }
}
async function messageIdDeTxHash({ rpc, txHash }) {
  const pr = nodo.proveedor(rpc);
  try { const r = await pr.getTransactionReceipt(txHash); return r ? messageIdDeRecibo(r) : null; }
  catch (_) { return null; } finally { pr.destroy(); }
}
// eth_call a mailbox.delivered(bytes32) — selector e495f1d4. Devuelve true/false, o null si no se pudo preguntar.
const SEL_DELIVERED = '0xe495f1d4';
function datosDelivered(messageId) {
  const h = messageIdHex(messageId); if (!h) throw new Error('messageId invalido');
  return SEL_DELIVERED + h.slice(2);
}
async function entregadoEnEvm({ rpc, mailbox, messageId }) {
  const pr = nodo.proveedor(rpc);
  try {
    const ret = await pr.call({ to: mailbox, data: datosDelivered(messageId) });
    if (!/^0x[0-9a-fA-F]{64}$/.test(ret)) return null;
    return BigInt(ret) === 1n;
  } catch (_) { return null; } finally { pr.destroy(); }
}
// (mailbox.delivered "<id base64url>") en la chain del puente. true/false, o null si el nodo no contesta.
async function entregadoEnKadena({ node, networkId, chain, messageId }) {
  const id = messageIdB64(messageId); if (!id) return null;
  try {
    const j = await kdaLocal(node, networkId, chain, `(${NS}.mailbox.delivered "${id}")`, [], '', false);
    const r = j.result || {};
    return r.status === 'success' && typeof r.data === 'boolean' ? r.data : null;
  } catch (_) { return null; }
}

module.exports = { dryRunKda2Evm, dryRunEvm2Kda, sendEvm2Kda, sendKda2Evm, getKadenaBalances, getEvmBalances, evmRecipient, NS,
  messageIdHex, messageIdB64, messageIdDeRecibo, messageIdDeRequestKey, messageIdDeTxHash, datosDelivered, entregadoEnEvm, entregadoEnKadena, TOPIC_DISPATCH_ID };
