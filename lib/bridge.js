...
async function kdaLocal(node, networkId, chain, code, signers, sender, preflight = true) {
  const o = { networkId, payload: { exec: { code, data: {} } }, signers: signers || [], meta: { chainId: String(chain), sender: sender || '', gasLimit: 150000, gasPrice: 1e-8, ttl: 600, creationTime: Math.floor(Date.now() / 1000) }, nonce: String(Date.now()) };
...
async function dryRunKda2Evm({ node, networkId, chain, senderAccount, senderPubKey, kadenaModule, evmDomain, recipientEvmAddr, amount }) {
  ...
  const signers = [{ pubKey: senderPubKey, clist: [{ name: 'coin.GAS', args: [] }, { name: `${NS}.${kadenaModule}.TRANSFER_REMOTE`, args: [evmDomain, senderAccount, rec, Number(amount)] }] }];
  const j = await kdaLocal(node, networkId, chain, code, signers, senderAccount);
  const pf = j.preflightResult; const result = pf ? pf.result : (j.result || j);
  return { code, result, gas: pf ? pf.gas : j.gas };
}
...
async function sendKda2Evm({ node, networkId, chain, senderAccount, senderPubKey, secretHex, kadenaModule, evmDomain, recipientEvmAddr, amount }, onStep = () => {}) {
  const rec = evmRecipient(recipientEvmAddr);
  const code = `(${NS}.mailbox.dispatch ${NS}.${kadenaModule} ${evmDomain} "${rec}" ${Number(amount).toFixed(12)})`;
  const clist = [{ name: 'coin.GAS', args: [] }, { name: `${NS}.${kadenaModule}.TRANSFER_REMOTE`, args: [evmDomain, senderAccount, rec, Number(amount)] }];
  const cmdObj = { networkId, payload: { exec: { code, data: {} } }, signers: [{ pubKey: senderPubKey, clist }], meta: { chainId: String(chain), sender: senderAccount, gasLimit: 60000, gasPrice: 1e-8, ttl: 600, creationTime: Math.floor(Date.now() / 1000) }, nonce: 'monedero-bridge:' + Date.now() };
