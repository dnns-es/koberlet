...
async function sendEvm2Kda({ rpc, secretHex, ledgerIndex, router, token, kadenaAccount, amount, decimals, kadenaDomain, kadenaChain }, onStep = () => {}) {
  ...
  const send = async (to, data, value) => {
    if (viaLedger) { const r = await ledger.sendContract({ index: ledgerIndex, rpc, to, data, valueWei: value || 0n }); return { hash: r.hash, wait: () => pr.waitForTransaction(r.hash) }; }
  ...
  if (allowance < amt) {
    ...
    out.approveHash = ap.hash; await ap.wait(); onStep({ step: 'approve', status: 'ok', detail: ap.hash });
  }
