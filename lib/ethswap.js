...
async function swap({ rpc, secretHex, dir, amount, minOut }, onStep = () => {}) {
  ...
  let min;
  if (minOut != null && Number(minOut) > 0) {
    min = ethers.parseUnits(Number(minOut).toFixed(decOut), decOut);
  } else {
    const outs = await r.getAmountsOut(amtIn, path); // compat: sin quote enlazado, cae al cálculo fresco
    min = outs[1] - (outs[1] * 5n / 1000n);
  }
