...
// Comision de servicio de Koberlet, la misma que en Kadena: 0,5%. Aqui NO hace falta
// contrato ni transferencia aparte, porque el propio router de Uniswap sabe repartir:
// `sweepTokenWithFee` y `unwrapWETH9WithFee` mandan una parte a otra cuenta dentro del
// mismo multicall. Es codigo suyo, auditado, y el router no admite mas del 1% (100
// bips), asi que ni por error se puede cobrar de mas.
//
// Diferencia con Kadena, y hay que decirla tal cual: alli la comision se aparta de lo
// que ENTRA; aqui sale de lo que SE RECIBE, porque es lo que el router sabe hacer.
const { FEE_BIPS, FEE_MAX_BIPS, ETH_CUENTA: CUENTA_DNNS, conComision } = require('./comision');
...
async function quote({ rpc, dir, amount }) {
  ...
  try {
    ...
    // `min` es el SUELO QUE SE FIRMA y va contra lo que da el pool, antes de la
    // comisión. Lo que el usuario recibe es eso menos la comisión: por eso se
    // devuelven las dos cifras y la pantalla enseña la segunda.
    const min = out - (out * 5n / 1000n);
    const { neto, comision } = conComision(out);
    const minNeto = conComision(min).neto;
    let gasEth = null;
    try { const f = await pr.getFeeData(); if (f.gasPrice) gasEth = Number(ethers.formatEther(f.gasPrice * GAS_SWAP)); } catch (_) {}
    const enNumero = (v) => Number(ethers.formatUnits(v, decOut));
    return {
      out: enNumero(neto), min: enNumero(min), minRecibes: enNumero(minNeto),
      bruto: enNumero(out), comision: enNumero(comision), comisionPct: Number(FEE_BIPS) / 100,
      gasEth, slipPct: SLIP * 100, fee
    };
  } finally { pr.destroy(); }
}
...
function partes({ dir, fee, amtIn, min, cuenta }) {
  ...
  if (FEE_BIPS > FEE_MAX_BIPS) throw new Error('La comisión no puede pasar del 1%.');
  if (dir === 'usdc2eth') {
    // El router se queda el WETH (ADDRESS_THIS) y lo desenvuelve a ETH nativo para el
    // usuario. El suelo se aplica al desenvolver, que revierte si sale menos, y en el
    // mismo paso se aparta la comisión: una sola transacción para las dos cosas.
    return [
      ...
      iface.encodeFunctionData('unwrapWETH9WithFee', [min, cuenta, FEE_BIPS, CUENTA_DNNS])
    ];
  }
  // ETH -> USDC: el ETH va como `value` y el router lo envuelve solo. El USDC se queda
  // un momento en el router para poder repartirlo, y `sweepTokenWithFee` lo saca todo
  // en la misma llamada: no queda nada dentro. refundETH devuelve lo que sobre, que si
  // no se quedaria atrapado en el contrato.
  return [
    iface.encodeFunctionData('exactInputSingle', [{ tokenIn, tokenOut, fee, recipient: ADDRESS_THIS, amountIn: amtIn, amountOutMinimum: min, sqrtPriceLimitX96: 0 }]),
    iface.encodeFunctionData('sweepTokenWithFee', [tokenOut, min, cuenta, FEE_BIPS, CUENTA_DNNS]),
...
module.exports = { quote, swap, partes, pathAndDec, conComision, ROUTER, USDC, WETH, FEE_BIPS, FEE_MAX_BIPS, CUENTA_DNNS };
