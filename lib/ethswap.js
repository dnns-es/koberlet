...
async function swap({ rpc, secretHex, dir, amount, minOut }, onStep = () => {}) {
  ...
  if (dir === 'usdc2eth') {
    ...
    if (allowance < amtIn) { onStep({ step: 'approve', status: 'run', detail: 'Autorizando USDC…' }); const ap = await t.approve(ROUTER, amtIn); out_.approveHash = ap.hash; await ap.wait(); onStep({ step: 'approve', status: 'ok', detail: ap.hash }); }
