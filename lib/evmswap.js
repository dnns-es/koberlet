...
// Comision de servicio de Koberlet, la misma que en el resto de la app. Aqui la aplica
// el propio router dentro del mismo multicall: el token sale del pool al router y
// `sweepTokenWithFee` lo reparte en la misma llamada, sin dejar nada dentro.
const { FEE_BIPS, FEE_MAX_BIPS, ETH_CUENTA: CUENTA_DNNS, conComision } = require('./comision');
// address(2) = "el propio router": hace falta para que pueda repartir lo que sale.
...
async function cotizar({ rpc, tokenIn, tokenOut, decIn, decOut, amount }) {
  ...
  try {
    ...
    // `salida` es lo que le llega al usuario: la comision ya esta descontada. La cifra
    // bruta se devuelve aparte porque es la que se firma como suelo del swap.
    const { neto, comision } = conComision(mejor.bruto);
    const salida = Number(ethers.formatUnits(neto, decOut));
    return {
      fee: mejor.fee, salida, salidaBruta: mejor.bruto.toString(),
      comision: Number(ethers.formatUnits(comision, decOut)), comisionPct: Number(FEE_BIPS) / 100,
      precio: salida / Number(amount)
    };
  } finally { pr.destroy(); }
}
...
async function swap({ rpc, secretHex, tokenIn, tokenOut, decIn, decOut, amount, slippage = 0.5 }, aviso = () => {}) {
  ...
  try {
    ...
    // --- Intercambio ---
    // El cambio y el reparto, en una sola transaccion: el token sale del pool al propio
    // router y `sweepTokenWithFee` lo saca entero -lo del usuario y la comision- en la
    // misma llamada. Si algo falla, falla todo y no se cobra nada.
    aviso('Cambiando en Uniswap…');
    if (FEE_BIPS > FEE_MAX_BIPS) throw new Error('La comision no puede pasar del 1%.');
    const router = new ethers.Contract(SWAP_ROUTER, ROUTER_ABI, firmante);
    const iface = new ethers.Interface(ROUTER_ABI);
    const datos = [
      iface.encodeFunctionData('exactInputSingle', [{
        tokenIn: tIn, tokenOut: tOut, fee: c.fee, recipient: ADDRESS_THIS,
        amountIn: entrada, amountOutMinimum: minimo, sqrtPriceLimitX96: 0
      }]),
      iface.encodeFunctionData('sweepTokenWithFee', [tOut, minimo, yo, FEE_BIPS, CUENTA_DNNS])
    ];
    const tx = await router.multicall(datos);
    const rec = await nodo.esperarRecibo(pr, tx.hash);
    if (!rec || rec.status !== 1) throw new Error('El intercambio fallo en la red.');
    const { neto } = conComision(minimo);
    return {
      hash: tx.hash, fee: c.fee,
      esperado: c.salida,
      minimo: Number(ethers.formatUnits(neto, decOut)),
      comision: c.comision, comisionPct: c.comisionPct,
